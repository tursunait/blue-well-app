...
async def analyze_image_with_openai(
    image_bytes: bytes, hint: Optional[str] = None
) -> Dict[str, Any]:
...
async def estimate_calories_from_text(food_description: str = Form(...)):
    """
...
Be as accurate as possible. Provide reasonable estimates based on typical serving sizes."""
        ...
        raise HTTPException(
            status_code=500, detail=f"Error estimating calories from text: {str(e)}"
        )
